...
def daterange(date1, date2):
    ...
    for n in range(1,int ((date2 - date1).days)+1):
        print('before yield --',date1 + timedelta(n))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.error(request,'zipcode must be in integer')
                return render(request, "dashboard/kyc_verification2.html", )
            ...
            if (request.POST['package']):
                ...
                if (cond):
                    ...
                    if (bal > 0 and bal >= amount):
                        ...
                        end_date = date_by_adding_business_days(start_date, int(days))
        ...
        if (obj.investment_package):
            ...
            if(obj.last_benefit_date!=datetime.today().date() and rem_days>0):
                ...
                if(balss.exists()):
                    ...
                    print('in investment @@ cl-->',cl,'rem_days --',rem_days)
                    bonus_am = round(cl*rem_days,2)
                    print('bonus profit ###',bonus_am)
                    objec.current_balance += float(bonus_am)
                    if(obj22):
                        print('obj22.daily_earnings 1 @@@@@@@', obj22.daily_earnings)
                        obj22.daily_earnings+=float(bonus_am)
                        print('obj22.daily_earnings 2 @@@@@@@', obj22.daily_earnings)
            ...
            print('pert partnership  %%%', pert,'t_days ###',t_days)
            t_amount = round(t_days * pert, 2)
            ...
            for dt in daterange(b, a):
                print('b ---',b,a)
                print('dt.weekday() ---dt',dt,dt.weekday())
                if dt.weekday() not in weekdays:  # to print only the weekdates
            ...
            rem_days = (a - b).days
            print('in partnership old rem days --',rem_days)
            rem_days = rem_days_count
            print('rem days @@@',rem_days)
            ...
            if (balss.exists()):
                ...
                if (mon_date_obj.month < datetime.today().month):
                    ...
                    #     user_obj.daily_earnings = round(obj22.daily_earnings, 2)
                    #     print("user_obj.monthly_royality_last_date 11 --",user_obj.monthly_royality_last_date)
                    #     user_obj.monthly_royality_last_date = datetime.today().date()
                    #     print("user_obj.monthly_royality_last_date 222 --", user_obj.monthly_royality_last_date)
                    #     user_obj.save()
                    # print('request.user.monthly_royality_last_date after--', request.user.email,user_obj.email)
                    # print('request.user.monthly_royality_last_date after--', request.user.monthly_royality_last_date,user_obj.monthly_royality_last_date)
            ...
            # mon_date_obj = request.user.monthly_royality_last_date
            # print('mon date obj ##', mon_date_obj, type(mon_date_obj),mon_date_obj.month)
            # print('date obj $$', dir(datetime.today()),datetime.today().month)
...
def withdraw_request(request):
    ...
    if(obb.exists()):
        ...
        if(obb.approved==False):
            # messages.error(request, 'Please complete KYC verification before withdraw')
            kyc_not_done=True
